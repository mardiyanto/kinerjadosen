  ...
  if (isset($_POST['simpan'])) {
    ...
    $JD=$_POST['JD'];
      ...
      if (mysqli_num_rows($cekagy) > 0) {
        echo "<script>alert('Data ".$nama." Sudah Tersedia!');window.location.href='admin.php?p=jabatan'</script>";
      }else{
        $input=mysqli_query($con,"INSERT INTO jabatan values (null, '$nama','$JD')");
        if ($input == TRUE) {
          echo "<script>alert('Data ".$nama." Berhasil Ditambahkan!');window.location.href='admin.php?p=jabatan'</script>";
        }else{
          echo "<script>alert('Data ".$nama." Gagal dieksekusi!');window.location.href='admin.php?p=jabatan&act=create'</script>";
        }
      }
  }
